...
import { X, MessageSquare, Users, Store, ShieldCheck, Radio, Target } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useSendInApp, useSendInAppBulk } from '../../hooks/useInAppNotifications'
import type { RecipientType, BroadcastSegment } from '../../types/notification.types'

// PENDING BACKEND — anticipated contract for POST /api/admin/notifications/inapp/send(/bulk).
// Mirrors SendPushModal's specific-vs-broadcast targeting; in-app inbox entries also carry an
// optional deep-link (actionUrl) the client app can navigate to when the entry is tapped.
const schema = z
  .object({
    ...
    actionUrl: z.string().max(300, 'Max 300 characters').optional(),
  })
...
export function SendInAppModal({ open, onClose }: Props) {
  const { mutate: sendInApp, isPending: sendingOne } = useSendInApp()
  const { mutate: sendInAppBulk, isPending: sendingBulk } = useSendInAppBulk()
  ...
  function onSubmit(values: FormValues) {
    const actionUrl = values.actionUrl?.trim() || undefined
    if (values.targetMode === 'specific') {
      ...
      sendInAppBulk(
        { segment: values.segment, title: values.title, message: values.message, actionUrl },
        { onSuccess: () => { reset(); onClose() } },
      )
    }
  }
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="font-semibold text-foreground">Compose In-App Notification</p>
                  <p className="text-xs text-muted-foreground">Appears in the recipient's in-app inbox</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              <div className="space-y-4 p-5">
                ...
                {targetMode === 'specific' ? (
                  ...
                  <div>
                    ...
                    <select className={INPUT_CLS} disabled={isPending} {...register('segment')}>
                      {SEGMENTS.map((s) => (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Write the in-app notification body…"
                    disabled={isPending}
                    {...register('message')}
                  />
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                    Action URL <span className="text-[10px] font-normal text-muted-foreground">(optional deep link)</span>
                  </label>
                  <input
                    type="text"
                    className={cn(INPUT_CLS, errors.actionUrl && 'border-destructive')}
                    placeholder="e.g. /coupons/42"
                    disabled={isPending}
                    {...register('actionUrl')}
                  />
                  {errors.actionUrl && <p className={ERROR_CLS} role="alert">{errors.actionUrl.message}</p>}
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-border px-5 py-4">
                ...
                >
                  ...
                  {isPending ? 'Sending…' : targetMode === 'broadcast' ? 'Broadcast' : 'Send'}
                </button>
              </div>
            </form>
          </motion.div>
